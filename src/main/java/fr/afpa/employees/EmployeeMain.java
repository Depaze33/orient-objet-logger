...
for (Employee employee : employee) {
 ...
 System.out.println("Les employee sont " + (employee)); 
		...
		try {
			...
			logger.info("Le matricule n'est pas bon");
		}
		try {
			employee1.setLastName("Azary");

		} catch (Exception e) {
			logger.info("Le nom n'est pas correcte ");
		...
		try {
			...
			logger.info("Le prénom n'est pas bon");
		...
		try {
			...
			logger.info("La date de naissance n'est pas bonne");
